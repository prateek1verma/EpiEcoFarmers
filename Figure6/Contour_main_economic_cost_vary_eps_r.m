...
parfor jj = 1:nn % Parallel loop over beta_vec
    ...
    eps_r = eps_r_vec(jj); % Efficacy of the fungicide on the resistant strain
    r_dummy_vec = r_vec;
...
Economic_cost_resistance = round(abs(Economic_cost_resistance),4);
imagesc([r_vec(1) r_vec(end)], [eps_r_vec(1) eps_r_vec(end)],Economic_cost_resistance) % displays the economic cost of resistance as a heatmap
...
ylabel('Efficacy of the fungicide, $$\epsilon_r$$','interpreter','latex',FontSize=fz) % sets the y-axis label with LaTeX formatting
...
hold on
[C1,h1] = imcontour([r_vec(1) r_vec(end)], [eps_r_vec(1) eps_r_vec(end)], Economic_cost_resistance,[0.35,0.3,0.2,0.1,10^-5],'--k','ShowText','on');
toc

writematrix(Economic_cost_resistance,'Contour_vary_eps_r.txt','Delimiter','tab')

% figure()
% [C1,h1] = imcontour([r_vec(1) r_vec(end)], [eps_r_vec(1) eps_r_vec(end)], Economic_cost_resistance,'ShowText','on');
% set(gca,'TickDir','out'); % sets the tick direction of the axes to outward
% % The only other option is 'in'
% set(gca,'FontSize',fz) % sets the font size of the axis labels and tick labels
% hax = gca; % gets the current axis handle
...
% colorbar % displays the colorbar
% hold on
% [C1,h1] = imcontour([r_vec(1) r_vec(end)], [eps_r_vec(1) eps_r_vec(end)], Economic_cost_resistance,'--k','ShowText','on');
