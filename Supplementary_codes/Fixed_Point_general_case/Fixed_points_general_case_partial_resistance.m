%% General case: Linear stablity analysis (with pesticide resistance)
disp('General Case: eps_w, eps_r')
syms Iwt Iwu Irt Iru beta_w beta_r N theta mu Iuw Itw Iur Itr epsilon_r epsilon_w
...
save('Fixedpoints_general_case.mat', 'fix_pt'); 

% [sol2.Iuw sol2.Itw sol2.Iur sol2.Itr]
% Fixed point 1
diary ('Fixedpoint1.txt');
...
diary ('StableCondition5.txt');
EigJ5 = (eig(simplify(J_new)))
diary off

% Eigenvalues for fixed point 5 
J = jacobian([Iuwdot Itwdot Iurdot Itrdot],[Iuw Itw Iur Itr]);
J_new = subs(J,{Iuw Itw Iur Itr},{fix_pt.Iuw(6) fix_pt.Itw(6) fix_pt.Iur(6) fix_pt.Itr(6)});

diary ('StableCondition6.txt');
EigJ6 = (eig(simplify(J_new)))
diary off
...
save('Eigenvalues_general_case.mat', 'Eigenvalue_vec'); 